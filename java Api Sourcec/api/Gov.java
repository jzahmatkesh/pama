...
public class Gov {
	...
	public Response gov(TBGov gov, @Context HttpServletRequest request)
	{
		JSONArray data = new JSONArray();
		try {
			DataBase db = new DataBase();
			Connection con = db.getConnection();
			PreparedStatement p = con.prepareStatement("Exec Pama.PrcGov ?,?,?");
	...
	@PUT
	@Produces(MediaType.APPLICATION_JSON+"; charset=UTF-8")
	public Response savegov(TBGov gov, @Context HttpServletRequest request)
	{
		try {
			DataBase db = new DataBase();
			Connection con = db.getConnection();
			PreparedStatement p = con.prepareStatement("Exec Pama.PrcSaveGov ?,?,?,?,?,?,?,?,?,?,?");
	 		p.setString(1, gov.getToken());
	 		p.setString(2, db.GetIPAddress(request));
	 		p.setString(3, db.BrowserInfo(request));
	 		p.setInt(4, gov.getid());
	 		p.setString(5, gov.getName());
	 		p.setString(6, gov.getFamily());
	 		p.setString(7, gov.getMobile());
	 		p.setString(8, gov.getTel());
	 		p.setString(9, gov.getPost());
	 		p.setString(10, gov.getAddress());
	 		p.setString(11, gov.getNote());
			java.sql.ResultSet rs = p.executeQuery();
			if (rs != null){
				while(rs.next())
	...
	@DELETE
	@Produces(MediaType.APPLICATION_JSON+"; charset=UTF-8")
	public Response delgov(@Context HttpServletRequest request, @HeaderParam("token") String token, @HeaderParam("id") int id)
	{
		try {
			DataBase db = new DataBase();
			Connection con = db.getConnection();
			PreparedStatement p = con.prepareStatement("Exec Pama.PrcDelGov ?,?,?,?");
	 		p.setString(1, token);
	 		p.setString(2, db.GetIPAddress(request));
	 		p.setString(3, db.BrowserInfo(request));
	 		p.setInt(4, id);
