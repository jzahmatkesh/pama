--Exec Pama.PrcCreate_CRUID 'Pama', 'TBGUnit', 'GUnit', 'GUnitMng'

Use Pama
Go
Drop Procedure If Exists Pama.PrcCreate_CRUID
Go
Create Procedure Pama.PrcCreate_CRUID @Schema NVarchar(10), @TBName NVarChar(100), @SPermission NVarChar(100), @UPermission NVarChar(100)
With Encryption
As
Begin
-- Exec Erp.PrcCreate_CRUID @Schema='Inv', @TBName='TBProduct', @SPermission='View_Product', @UPermission = 'Edit_Product'
	...
	Begin
		if @PKUpdate is Null
			...
			Set @PKUpdate = @PKUpdate+' And '+@FieldName+' = @'+@FieldName
	...
	Insert Into @Res Select '	set nocount on;'
	Insert Into @Res Select '	if Not Exists(Select * From Pama.TBUsers Where Token=@Token And TokenTime+3 >= GetDate() And IsNull(Active, 0) = 1)'
	Insert Into @Res Select '		Throw 50000, ''Token Not Valid'', 1'
	Insert Into @Res Select '	if Not Exists(Select * From Pama.FnUserPermission(@Token, '''+@SPermission+'''))'
	...
	Insert Into @Res Select 'Go'
	Insert Into @Res Select 'Grant Execute On '+@Schema+'.PrcView_'+REPLACE(@TBName, 'TB', '')+' To Pama'
	Insert Into @Res Select 'Go'
	...
	Insert Into @Res Select '	if Not Exists(Select * From Pama.TBUsers Where Token=@Token And TokenTime+3 >= GetDate() And IsNull(Active, 0) = 1)'
	Insert Into @Res Select '		Throw 50000, ''Token Not Valid'', 1'
	Insert Into @Res Select '	if Not Exists(Select * From Pama.FnUserPermission(@Token, '''+@UPermission+'''))'
	Insert Into @Res Select '		Throw 50001, ''You dont have permission'', 1'
	...
	Insert Into @Res Select 'End'
	Insert Into @Res Select 'Go'
	Insert Into @Res Select 'Grant Execute On '+@Schema+'.PrcSave_'+REPLACE(@TBName, 'TB', '')+' To Pama'
	Insert Into @Res Select 'Go'
	...
	Insert Into @Res Select 'End'
	Insert Into @Res Select 'Go'
	Insert Into @Res Select 'Grant Execute On '+@Schema+'.PrcDel_'+REPLACE(@TBName, 'TB', '')+' To Pama'
	Insert Into @Res Select 'Go'
